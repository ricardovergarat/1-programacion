...
def start_curve(a=1,t=4*np.pi,anima="ON",contador=0):
	...
	Button(principal_frame,text="ON",command=lambda:recuperar(ventana,entry_a,entry_t,"ON",contador)).place(x=830,y=50)
	Button(principal_frame,text="OFF",command=lambda:recuperar(ventana,entry_a,entry_t,"OFF",contador)).place(x=860,y=50)
	# ------------------------------- PRINCIPAL FRAME -------------------
	# ------------------------------- BOTONES FRAME -------------------
	# botones
	...
	Button(grafico_frame,text="Graficar",command=lambda:recuperar(ventana,entry_a,entry_t,anima,contador)).place(x=800,y=823)
	# implementar grafico
	print("Evaluar")
	print("a=",a)
	print("t=",t)
	...
	if anima == "ON":
		...
		animacion = animation.FuncAnimation(fig, muerte_a_omar, fargs=(ejes, linea),frames=61,interval=16.6, blit=False,)
	else:
		linea, = grafico.plot(x, y, z, 'm', label="Curva de Viviani", lw=1)
...
def recuperar(ventana,entry_a,entry_t,anima,contador):
	...
	start_curve(a,t,anima,contador)
