...
def start_curve(a=1,t=4*np.pi,anima="ON",contador=0,presionado="vacio"):
	...
	if anima == "ON":
		...
		#animacion = animation.FuncAnimation(fig, muerte_a_omar, fargs=(ejes, linea2),frames=61,interval=16.6, blit=False,repeat=False)
	else:
		linea, = grafico.plot(x, y, z, 'm', label="Curva de Viviani", lw=1)
...
def operacion(presionado,a,t):
	if presionado == "Posicion":
		...
		return "Estamos trabajando para ello"
